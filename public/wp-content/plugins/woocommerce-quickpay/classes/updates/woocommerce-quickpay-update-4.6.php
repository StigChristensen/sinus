    ...
    foreach( $subscriptions as $subscription_post ) {
        ...
        if (!empty($transaction_id) && $order->has_quickpay_payment()) {
            ...
            try {
                ...
                delete_post_meta($order->id, 'TRANSACTION_ID', $transaction_id);

                $logger->add(sprintf('Migrated transaction (%d) from parent order ID: %s to subscription order ID: %s', $transaction_id, $subscription->id, $order->id));
            } catch( QuickPay_API_Exception $e ) {
                $logger->add(sprintf('Failed migration of transaction (%d) from parent order ID: %s to subscription order ID: %s. Error: %s', $transaction_id, $subscription->id, $order->id, $e-getMessage()));
            }
        }
    }
}
